...
|---|---|
| `Once[a,b] p` / internal text `O[a,b] p` | true iff some offset `j` in `[a,b]` makes `p` true at `t-j` |
| `Historically[a,b] p` / internal text `H[a,b] p` | true iff every offset `j` in `[a,b]` makes `p` true at `t-j` |
| `StrongPrevious p` / internal text `Y p` | exactly the `Once[1,1] p` truth relation selected by `ix://agent-ix/quire-specification/FR-092`; it is primitive and does not imply a weak or physical-position-existence boundary |
| `p Since[a,b] q` / internal text `p S[a,b] q` | true iff some witness offset `j` in `[a,b]` makes `q` true at `t-j` and `p` is true at every offset `k` in `[a,j)` |
| `p Triggered[a,b] q` / internal text `p T[a,b] q` | exactly the Boolean dual `not((not p) S[a,b] (not q))` |
...
The semantic/Rust node labels are `Once`, `Historically`, `StrongPrevious`,
`Since`, and `Triggered`; they are not accepted long-form text spellings. The
internal v3 dialect accepts only the case-sensitive `O`, `H`, `Y`, `S`, and `T`
spellings. The v1 profile refuses weak Previous. It also refuses every future
...
| FR-011-AC-4 | Weak Previous, every future or mixed-time node, every unknown past operator, and every unknown operator-profile identity are rejected before evaluation under the past profile. | Test (TC-053) |
| FR-011-AC-5 | Strong Previous is a distinct primitive node whose truth equals `Once[1,1]` for propositions, constants and Boolean nests at every anchor including origin; weak Previous and long-form text spellings refuse. | Test (TC-050, TC-053) |
