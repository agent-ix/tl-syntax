...
# FR-018: Require an exact human source-release decision
...
|---|---|---|
| FR-018-AC-1 | Accepted/rejected/deferred/conditional/open/conflict remain distinct exact-subject dispositions; event-backed states retain identity/rationale/exception/expiry, `open` requires no absent event fields, and `conflict` retains the complete event set. | Test (TC-062) |
| FR-018-AC-2 | An absent required review/decision or an incomplete, cross-snapshot, changed-cursor or over-bound event enumeration leaves the disposition `open`; a stale or wrong-actor/subject event is `refused` while the disposition stays `open`; an ambiguous/contradictory complete current set produces `conflict`, and none is inferred as accepted. | Test (TC-063) |
| FR-018-AC-3 | Every material candidate/configuration/profile/limitation/shared-contract change requires a new review/decision relation and preserves the historical disposition. | Test (TC-063) |
| FR-018-AC-4 | A source-release acceptance grants neither publication nor integrator/system/monitor qualification. | Test (TC-066) |
| FR-018-AC-5 | Each required review binds an attributable independent reviewer, exact head/base/configuration/evidence set, finding dispositions and accepted limitations; self-review or any bound-identity change leaves the review requirement open. | Test (TC-073) |
| FR-018-AC-6 | Independence and authority are evaluated against an exact policy/version, author/contributor set, conflicts, delegations, revocations and quorum; an unverifiable event is `refused`, and an insufficient quorum leaves the disposition `open`. | Test (TC-073) |
| FR-018-AC-7 | Conditional decisions retain condition owner/evidence/expiry; unresolved and satisfied conditions stay `conditional` until a new authorized decision; expiry reopens the disposition only at a verified equal-or-later instant under the bound clock/time authority and canonical ordering; deferred decisions retain owner/resume/expiry until a new authorized decision; and concurrent/contradictory successors produce `conflict` until explicitly resolved. | Test (TC-070) |
