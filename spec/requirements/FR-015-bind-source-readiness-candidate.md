...
# FR-015: Bind source-readiness facts to one candidate and configuration
...
|---|---|---|
| FR-015-AC-1 | Every healthy readiness input names one exact candidate revision, complete tracked-source set, crate/package bytes, feature/target/build configuration, corpus/profile revision, suite set and compatible shared-contract artifact set. | Test (TC-059) |
| FR-015-AC-2 | Mutating any identity or digest changes the readiness subject; omitting a required candidate identity is refused, while absence/incompatibility of a required shared capability is unavailable, and no prior result is reused. | Test (TC-060) |
| FR-015-AC-3 | Dirty, untracked-live, mutable-ignore, incomplete-census and unavailable-enumeration cases cannot be represented as a clean reviewed candidate. | Test (TC-060) |
| FR-015-AC-4 | Absence of a compatible released source-grounding contract remains unavailable and creates no local parser, schema, matrix or branch-head substitute. | Test (TC-066) |
| FR-015-AC-5 | Every `ExecutionOutcome` value in MRS-004 is exercised with a valid stale output present; no non-`succeeded` value yields a current successful domain output, every accepted output binds fresh invocation/executable/termination facts, and mid-run plus change-then-restore source/configuration mutations are refused. | Test (TC-068) |
| FR-015-AC-6 | Candidate identity includes repository/Git/materialized-source/admitted-root identities; the exact descriptor-backed bytes that are identified are consumed without pathname reopen; material or root/path/mount substitution/race is detected or unavailable; symlink chains below/at AP-002's bound terminate; and dangling/cyclic/out-of-root/special/mount-crossing/over-bound paths are refused. | Test (TC-069) |
