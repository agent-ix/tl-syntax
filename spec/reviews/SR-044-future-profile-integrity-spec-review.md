...
| FND-4406 | medium | Request identity was checked but absent from the raw schema, and a caller-selected document limit conflicted with formula-v1's fixed bound. Fixed by adding bounded raw request-identity bytes and deriving both the base count and fixed 100,000-node limit from owning contracts. | FR-008, FR-004, TC-046 |
