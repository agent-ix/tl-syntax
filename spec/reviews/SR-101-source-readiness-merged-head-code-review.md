...
| --- | --- | --- | --- | --- |
| FND-7501 | medium | Merging current main initially required a manual source-census union. The resolved population preserves every landed W/M source and fixture as well as the M6 fail-closed archival control; the exact 128-path and per-area assertions pass. | `tests/shared_assurance.rs`, PR #43, SR-100 | implementation-bug-despite-evidence |
| FND-7502 | low | SR-094's matrix-header finding is superseded by the single `Status` contract landed on main. TM-004 now uses that contract, validates without identifier/status warnings and remains honestly planned at 0 of 17 rows. | TM-004, SR-094, `9598fea` | correct-requirement-no-evidence |
