...
- [ ] **FR-015**: Bind every fact to one immutable candidate/configuration and fresh producer execution.
- [ ] **FR-016**: Preserve lifecycle stages, authority, retention and supersession without promotion.
- [ ] **FR-017**: Emit a lossless integrator package only through the accepted shared contract.
- [ ] **FR-018**: Admit an exact, policy-authorized human source-release disposition.
- [ ] **FR-019**: Classify every executable readiness path and remove stable reliance on legacy paths.
...
- `M6 specification + review -> every implementation task`
  Reason: the specification snapshot and SR-095 through SR-102 establish the
  boundary and test vocabulary before code is authorized.
- `external prerequisite admission -> FR-015, FR-016, FR-017, FR-018, FR-019`
