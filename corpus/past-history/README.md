...
The parser, evaluator, and rewrite repositories read this corpus in place
through `tl_syntax::CORPUS_DIR` and pin the manifest digest in their own
replay evidence; none retains a copy. Their native replay tests consume only
the fields owned by that component and still validate the complete closed
corpus shape.
